...
render = ->
  ...
  inspect = (stadium, index) ->
    ...
    info.append('span')
      .attr('class', 'row team-name')
      .html((d) -> "#{d.team} #{d.nickname}")

    info.append('span')
      .attr('class', 'row team-location')
      .html((d) -> "#{d.city}, #{d.state}")

    built = info.append('span')
      .attr('class', 'row range-chart')
  ...
  d3.csv 'data/stadiums.csv', format, (err, data) ->
